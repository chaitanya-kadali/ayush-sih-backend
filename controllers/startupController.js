...
exports.StartupLogin = catchAsyncErrors(async (req, res) => {
  ...
  try {
    ...
    if (!StartupDetails) {
      ...
      return res.status(404).json({
        success: false,
        throwmsg: "email not found !!",
        error: "User not found or Invalid Email_ID.",
      });
    }
    // Compare passwords
    const passwordMatch = await bcrypt.compare(
      password,
      StartupDetails.password
    );
    if (!passwordMatch) {
      ...
      return res.status(403).json({
        success: false,
        throwmsg: "Password did'nt match !!",
...
exports.Startup_Dashboard_Create = catchAsyncErrors(async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      success: true,
      message: "Dashboard details successfully saved",
      newStartupdashModel: newStartupdashModel,
    });
  } catch (error) {
    console.error("Error:", error);
...
exports.Startup_farmer_tab = catchAsyncErrors(async (req, res) => {
  ...
  try {
    ...
    if (!startup) {
      ...
      return res.status(404).json({
        startupfound: false,
        error: "No Startups Available. with the sent email",
      }); // no possible chances for this to happen
    }
    ...
    if (FarmersAvail.length === 0) {
      ...
      return res.status(200).json({
        farmerRetrievalSuccess: false,
        message: "No Farmers Available in this Startups district.",
      }); // status should be 200 as no farmer is not an error
    }
    console.log("here i am :", FarmersAvail);

    res
      .status(200)
      .json({ farmerRetrievalSuccess: true, Farmerslist: FarmersAvail }); // considering tokensuccess : true was already sent before
  } catch (error) {
    console.error(
      "Error during farmer data fetch at startup dashboard:",
      error
    );
    res.status(500).json({ success: false, error: "Internal server error" });
  }
...
exports.Startup_docter_tab = catchAsyncErrors(async (req, res) => {
  ...
  try {
    ...
    if (!startup) {
      ...
      return res
        .status(404)
        .json({ success: false, error: "No Startup Available." });
    }
    const DoctorsAvai = await Doctor.find({ district: startup.district });

    if (DoctorsAvai.lenght === 0) {
      res.status(404).json({
        DoctorRetrievalSuccess: false,
        error: "No Doctors Available in this Startup's district.",
      });
    }

    res.json({
      success: true,
      DoctorRetrievalSuccess: true,
      message: "The Doctors Details for Startup",
      DoctorsAvai: DoctorsAvai,
    });
  } catch (error) {
  ...
  try {
    ...
    if (!VerifyEmail) {
      ...
      return res.status(202).json({
        success: false,
        message: "Startup Dash Model Doesn't exist",
        data: [],
      });
    }
...
exports.StartupBaisic = catchAsyncErrors(async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Startup found ", basicdata: StartUp });
  } catch (error) {
